...
El diseño vive en `chuleta.js`, no aquí. Este módulo solo arma el archivo autónomo:
...
JS = (Path(__file__).with_name("chuleta.js")).read_text(encoding="utf-8")
...
<title>{e(documento.get('titulo',''))} — chuleta</title></head><body>
<script>const DOC={datos};</script>
<script>{COMUN}</script>
<script>{JS}</script>
<script>ForjaChuleta.montar(DOC);</script>
</body></html>"""
